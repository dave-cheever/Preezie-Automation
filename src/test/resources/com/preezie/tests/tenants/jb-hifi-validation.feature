@tenant-jb-hifi
Feature: Chat API Validation - JB HIFI
  # Test data is loaded from external CSV: testdata/JB_HIFI.csv
...
Background:
  ...
  * def tenantId = 'tnt_sJaLLkeEMDVUI9G'
  * def tenantName = 'JB HIFI'
  # Load and filter test data - only enabled rows
  * def TestDataFilter = Java.type('com.preezie.utils.TestDataFilter')
  * def enabledTestData = TestDataFilter.getEnabledTestData('testdata/JB_HIFI.csv')
  * def testCount = karate.sizeOf(enabledTestData)
  * karate.log('Enabled test cases for JB HIFI:', testCount)

Scenario: Run all enabled JB HIFI tests
  * if (testCount == 0) karate.fail('No enabled test cases found in JB_HIFI.csv')
