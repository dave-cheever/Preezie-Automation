@tenant-puma
Feature: Chat API Validation - PUMA
  # Test data is loaded from external CSV: testdata/PUMA.csv
...
Background:
  ...
  * def tenantId = 'tnt_sZSeICB9hop90GD'
  * def tenantName = 'PUMA'
  # Load and filter test data - only enabled rows
  * def TestDataFilter = Java.type('com.preezie.utils.TestDataFilter')
  * def enabledTestData = TestDataFilter.getEnabledTestData('testdata/PUMA.csv')
  * def testCount = karate.sizeOf(enabledTestData)
  * karate.log('Enabled test cases for PUMA:', testCount)

Scenario: Run all enabled PUMA tests
  * if (testCount == 0) karate.fail('No enabled test cases found in PUMA.csv')
