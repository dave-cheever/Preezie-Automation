Feature: Run evaluator against specificProductQuestionResponse

Scenario:
  * karate.log('=== RUN-SPECIFICPRODUCTQUESTIONRESPONSE-EVALUATOR.FEATURE STARTED ===')
  ...
  * def evaluatorSystem = read('classpath:com/preezie/llm/validators/prompts/specificproductquestionresponse-evaluator-system.prompt.txt')
  * def evaluatorUser = read('classpath:com/preezie/llm/validators/prompts/specificproductquestionresponse-evaluator-user.prompt.txt')
  ...
  * karate.log('=== CALLING LLM-CLIENT.FEATURE FOR SPECIFICPRODUCTQUESTIONRESPONSE EVALUATION ===')
  ...
  * karate.log('=== SpecificProductQuestionResponse LLM call succeeded:', llmCallSucceeded)

  * def evaluatorResult = llmResult.evaluatorResponse
  * karate.log('=== SpecificProductQuestionResponse evaluatorResponse received, has usage:', evaluatorResult && evaluatorResult.usage ? 'yes' : 'no')

  # If LLM call failed, create a dummy validation result
  * eval
    """
    if (!llmCallSucceeded) {
      karate.log('=== SPECIFICPRODUCTQUESTIONRESPONSE LLM CALL FAILED - Creating fallback result ===');
      ...
      evaluatorResult.parsedContent = {
        ...
        summary: 'SpecificProductQuestionResponse evaluation could not be performed due to API error'
      };
    } else {
  ...
  # Record usage - directly write to CSV using Java (for specificProductQuestionResponse AI cost tracking)
  * print '=== BEFORE SPECIFICPRODUCTQUESTIONRESPONSE USAGE RECORDING - evaluatorResult type:', typeof evaluatorResult
  * eval
    """
    karate.log('=== INSIDE SPECIFICPRODUCTQUESTIONRESPONSE USAGE RECORDING EVAL ===');
    if (evaluatorResult && evaluatorResult.usage) {
      try {
        ...
        karate.log('=== RECORDING SPECIFICPRODUCTQUESTIONRESPONSE USAGE ===');
        ...
        builder.content(content + ' [specificProductQuestionResponse]');
        ...
        karate.log('=== SPECIFICPRODUCTQUESTIONRESPONSE USAGE RECORDED SUCCESSFULLY ===');
      } catch (e) {
        karate.log('ERROR recording specificProductQuestionResponse usage:', e);
      }
    } else {
      karate.log('WARNING: No usage data found in specificProductQuestionResponse evaluatorResult');
    }
    """
  ...
  * def specificProductQuestionResponseEvaluatorResultOut = evaluatorResult
  * def specificProductQuestionResponseValidationOut = validation
  * def specificProductQuestionResponseLlmCallSucceededOut = llmCallSucceeded
