Feature: Run evaluator against specificProductQuestion response

Scenario:
  * karate.log('=== RUN-SPECIFICPRODUCTQUESTION-EVALUATOR.FEATURE STARTED ===')
  ...
  * def evaluatorSystem = read('classpath:com/preezie/llm/validators/prompts/specificproductquestion-evaluator-system.prompt.txt')
  * def evaluatorUser = read('classpath:com/preezie/llm/validators/prompts/specificproductquestion-evaluator-user.prompt.txt')
  ...
  * karate.log('=== CALLING LLM-CLIENT.FEATURE FOR SPECIFICPRODUCTQUESTION EVALUATION ===')
  ...
  * karate.log('=== SpecificProductQuestion LLM call succeeded:', llmCallSucceeded)

  * def evaluatorResult = llmResult.evaluatorResponse
  * karate.log('=== SpecificProductQuestion evaluatorResponse received, has usage:', evaluatorResult && evaluatorResult.usage ? 'yes' : 'no')

  # If LLM call failed, create a dummy validation result
  * eval
    """
    if (!llmCallSucceeded) {
      karate.log('=== SPECIFICPRODUCTQUESTION LLM CALL FAILED - Creating fallback result ===');
      ...
      evaluatorResult.parsedContent = {
        ...
        summary: 'SpecificProductQuestion evaluation could not be performed due to API error'
      };
    } else {
  ...
  # Record usage - directly write to CSV using Java (for specificProductQuestion AI cost tracking)
  * print '=== BEFORE SPECIFICPRODUCTQUESTION USAGE RECORDING - evaluatorResult type:', typeof evaluatorResult
  * eval
    """
    karate.log('=== INSIDE SPECIFICPRODUCTQUESTION USAGE RECORDING EVAL ===');
    if (evaluatorResult && evaluatorResult.usage) {
      try {
        ...
        karate.log('=== RECORDING SPECIFICPRODUCTQUESTION USAGE ===');
        ...
        builder.content(content + ' [specificProductQuestion]');
        ...
        karate.log('=== SPECIFICPRODUCTQUESTION USAGE RECORDED SUCCESSFULLY ===');
      } catch (e) {
        karate.log('ERROR recording specificProductQuestion usage:', e);
      }
    } else {
      karate.log('WARNING: No usage data found in specificProductQuestion evaluatorResult');
    }
    """
  ...
  * def specificProductQuestionEvaluatorResultOut = evaluatorResult
  * def specificProductQuestionValidationOut = validation
  * def specificProductQuestionLlmCallSucceededOut = llmCallSucceeded
