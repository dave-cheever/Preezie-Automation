Feature: Run evaluator against specificProductSizeRecommendation

Scenario:
  * karate.log('=== RUN-SPECIFICPRODUCTSIZERECOMMENDATION-EVALUATOR.FEATURE STARTED ===')
  ...
  * def evaluatorSystem = read('classpath:com/preezie/llm/validators/prompts/specificproductsizerecommendation-evaluator-system.prompt.txt')
  * def evaluatorUser = read('classpath:com/preezie/llm/validators/prompts/specificproductsizerecommendation-evaluator-user.prompt.txt')
  ...
  * karate.log('=== CALLING LLM-CLIENT.FEATURE FOR SPECIFICPRODUCTSIZERECOMMENDATION EVALUATION ===')
  ...
  * karate.log('=== SpecificProductSizeRecommendation LLM call succeeded:', llmCallSucceeded)

  * def evaluatorResult = llmResult.evaluatorResponse
  * karate.log('=== SpecificProductSizeRecommendation evaluatorResponse received, has usage:', evaluatorResult && evaluatorResult.usage ? 'yes' : 'no')

  # If LLM call failed, create a dummy validation result
  * eval
    """
    if (!llmCallSucceeded) {
      karate.log('=== SPECIFICPRODUCTSIZERECOMMENDATION LLM CALL FAILED - Creating fallback result ===');
      ...
      evaluatorResult.parsedContent = {
        ...
        summary: 'SpecificProductSizeRecommendation evaluation could not be performed due to API error'
      };
    } else {
  ...
  # Record usage - directly write to CSV using Java (for specificProductSizeRecommendation AI cost tracking)
  * print '=== BEFORE SPECIFICPRODUCTSIZERECOMMENDATION USAGE RECORDING - evaluatorResult type:', typeof evaluatorResult
  * eval
    """
    karate.log('=== INSIDE SPECIFICPRODUCTSIZERECOMMENDATION USAGE RECORDING EVAL ===');
    if (evaluatorResult && evaluatorResult.usage) {
      try {
        ...
        karate.log('=== RECORDING SPECIFICPRODUCTSIZERECOMMENDATION USAGE ===');
        ...
        builder.content(content + ' [specificProductSizeRecommendation]');
        ...
        karate.log('=== SPECIFICPRODUCTSIZERECOMMENDATION USAGE RECORDED SUCCESSFULLY ===');
      } catch (e) {
        karate.log('ERROR recording specificProductSizeRecommendation usage:', e);
      }
    } else {
      karate.log('WARNING: No usage data found in specificProductSizeRecommendation evaluatorResult');
    }
    """
  ...
  * def specificProductSizeRecommendationEvaluatorResultOut = evaluatorResult
  * def specificProductSizeRecommendationValidationOut = validation
  * def specificProductSizeRecommendationLlmCallSucceededOut = llmCallSucceeded
