Feature: Run evaluator against productCompareResponse

Scenario:
  * karate.log('=== RUN-PRODUCTCOMPARERESPONSE-EVALUATOR.FEATURE STARTED ===')
  ...
  * def evaluatorSystem = read('classpath:com/preezie/llm/validators/prompts/productcompareresponse-evaluator-system.prompt.txt')
  * def evaluatorUser = read('classpath:com/preezie/llm/validators/prompts/productcompareresponse-evaluator-user.prompt.txt')
  ...
  * karate.log('=== CALLING LLM-CLIENT.FEATURE FOR PRODUCTCOMPARERESPONSE EVALUATION ===')
  ...
  * karate.log('=== ProductCompareResponse LLM call succeeded:', llmCallSucceeded)

  * def evaluatorResult = llmResult.evaluatorResponse
  * karate.log('=== ProductCompareResponse evaluatorResponse received, has usage:', evaluatorResult && evaluatorResult.usage ? 'yes' : 'no')

  # If LLM call failed, create a dummy validation result
  * eval
    """
    if (!llmCallSucceeded) {
      karate.log('=== PRODUCTCOMPARERESPONSE LLM CALL FAILED - Creating fallback result ===');
      ...
      evaluatorResult.parsedContent = {
        ...
        summary: 'ProductCompareResponse evaluation could not be performed due to API error'
      };
    } else {
  ...
  # Record usage - directly write to CSV using Java (for productCompareResponse AI cost tracking)
  * print '=== BEFORE PRODUCTCOMPARERESPONSE USAGE RECORDING - evaluatorResult type:', typeof evaluatorResult
  * eval
    """
    karate.log('=== INSIDE PRODUCTCOMPARERESPONSE USAGE RECORDING EVAL ===');
    if (evaluatorResult && evaluatorResult.usage) {
      try {
        ...
        karate.log('=== RECORDING PRODUCTCOMPARERESPONSE USAGE ===');
        ...
        builder.content(content + ' [productCompareResponse]');
        ...
        karate.log('=== PRODUCTCOMPARERESPONSE USAGE RECORDED SUCCESSFULLY ===');
      } catch (e) {
        karate.log('ERROR recording productCompareResponse usage:', e);
      }
    } else {
      karate.log('WARNING: No usage data found in productCompareResponse evaluatorResult');
    }
    """
  ...
  * def productCompareResponseEvaluatorResultOut = evaluatorResult
  * def productCompareResponseValidationOut = validation
  * def productCompareResponseLlmCallSucceededOut = llmCallSucceeded
