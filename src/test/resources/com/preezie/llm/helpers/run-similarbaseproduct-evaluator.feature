Feature: Run evaluator against similarBaseProduct

Scenario:
  * karate.log('=== RUN-SIMILARBASEPRODUCT-EVALUATOR.FEATURE STARTED ===')
  ...
  * def evaluatorSystem = read('classpath:com/preezie/llm/validators/prompts/similarbaseproduct-evaluator-system.prompt.txt')
  * def evaluatorUser = read('classpath:com/preezie/llm/validators/prompts/similarbaseproduct-evaluator-user.prompt.txt')
  ...
  * karate.log('=== CALLING LLM-CLIENT.FEATURE FOR SIMILARBASEPRODUCT EVALUATION ===')
  ...
  * karate.log('=== SimilarBaseProduct LLM call succeeded:', llmCallSucceeded)

  * def evaluatorResult = llmResult.evaluatorResponse
  * karate.log('=== SimilarBaseProduct evaluatorResponse received, has usage:', evaluatorResult && evaluatorResult.usage ? 'yes' : 'no')

  # If LLM call failed, create a dummy validation result
  * eval
    """
    if (!llmCallSucceeded) {
      karate.log('=== SIMILARBASEPRODUCT LLM CALL FAILED - Creating fallback result ===');
      ...
      evaluatorResult.parsedContent = {
        ...
        summary: 'SimilarBaseProduct evaluation could not be performed due to API error'
      };
    } else {
  ...
  # Record usage - directly write to CSV using Java (for similarBaseProduct AI cost tracking)
  * print '=== BEFORE SIMILARBASEPRODUCT USAGE RECORDING - evaluatorResult type:', typeof evaluatorResult
  * eval
    """
    karate.log('=== INSIDE SIMILARBASEPRODUCT USAGE RECORDING EVAL ===');
    if (evaluatorResult && evaluatorResult.usage) {
      try {
        ...
        karate.log('=== RECORDING SIMILARBASEPRODUCT USAGE ===');
        ...
        builder.content(content + ' [similarBaseProduct]');
        ...
        karate.log('=== SIMILARBASEPRODUCT USAGE RECORDED SUCCESSFULLY ===');
      } catch (e) {
        karate.log('ERROR recording similarBaseProduct usage:', e);
      }
    } else {
      karate.log('WARNING: No usage data found in similarBaseProduct evaluatorResult');
    }
    """
  ...
  * def similarBaseProductEvaluatorResultOut = evaluatorResult
  * def similarBaseProductValidationOut = validation
  * def similarBaseProductLlmCallSucceededOut = llmCallSucceeded
