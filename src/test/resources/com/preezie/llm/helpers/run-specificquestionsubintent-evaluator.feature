Feature: Run evaluator against getSpecificQuestionSubIntent classification

Scenario:
  * karate.log('=== RUN-SPECIFICQUESTIONSUBINTENT-EVALUATOR.FEATURE STARTED ===')
  ...
  * def evaluatorSystem = read('classpath:com/preezie/llm/validators/prompts/specificquestionsubintent-evaluator-system.prompt.txt')
  * def evaluatorUser = read('classpath:com/preezie/llm/validators/prompts/specificquestionsubintent-evaluator-user.prompt.txt')
  ...
  * karate.log('=== CALLING LLM-CLIENT.FEATURE FOR SPECIFICQUESTIONSUBINTENT EVALUATION ===')
  ...
  * karate.log('=== SpecificQuestionSubIntent LLM call succeeded:', llmCallSucceeded)

  * def evaluatorResult = llmResult.evaluatorResponse
  * karate.log('=== SpecificQuestionSubIntent evaluatorResponse received, has usage:', evaluatorResult && evaluatorResult.usage ? 'yes' : 'no')

  # If LLM call failed, create a dummy validation result
  * eval
    """
    if (!llmCallSucceeded) {
      karate.log('=== SPECIFICQUESTIONSUBINTENT LLM CALL FAILED - Creating fallback result ===');
      ...
      evaluatorResult.parsedContent = {
        ...
        summary: 'SpecificQuestionSubIntent evaluation could not be performed due to API error'
      };
    } else {
  ...
  # Record usage - directly write to CSV using Java (for getSpecificQuestionSubIntent AI cost tracking)
  * print '=== BEFORE SPECIFICQUESTIONSUBINTENT USAGE RECORDING - evaluatorResult type:', typeof evaluatorResult
  * eval
    """
    karate.log('=== INSIDE SPECIFICQUESTIONSUBINTENT USAGE RECORDING EVAL ===');
    if (evaluatorResult && evaluatorResult.usage) {
      try {
        ...
        karate.log('=== RECORDING SPECIFICQUESTIONSUBINTENT USAGE ===');
        ...
        builder.content(content + ' [getSpecificQuestionSubIntent]');
        ...
        karate.log('=== SPECIFICQUESTIONSUBINTENT USAGE RECORDED SUCCESSFULLY ===');
      } catch (e) {
        karate.log('ERROR recording specificQuestionSubIntent usage:', e);
      }
    } else {
      karate.log('WARNING: No usage data found in specificQuestionSubIntent evaluatorResult');
    }
    """
  ...
  * def specificQuestionSubIntentEvaluatorResultOut = evaluatorResult
  * def specificQuestionSubIntentValidationOut = validation
  * def specificQuestionSubIntentLlmCallSucceededOut = llmCallSucceeded
