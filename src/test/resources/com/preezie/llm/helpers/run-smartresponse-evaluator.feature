Feature: Run smartResponse evaluator against smartResponse

Scenario:
  * karate.log('=== RUN-SMARTRESPONSE-EVALUATOR.FEATURE STARTED ===')
  ...
  * def evaluatorSystem = read('classpath:com/preezie/llm/validators/prompts/smartresponse-evaluator-system.prompt.txt')
  * def evaluatorUser = read('classpath:com/preezie/llm/validators/prompts/smartresponse-evaluator-user.prompt.txt')
  ...
  * karate.log('=== CALLING LLM-CLIENT.FEATURE FOR SMARTRESPONSE EVALUATION ===')
  ...
  * karate.log('=== SmartResponse LLM call succeeded:', llmCallSucceeded)

  * def evaluatorResult = llmResult.evaluatorResponse
  * karate.log('=== SmartResponse evaluatorResponse received, has usage:', evaluatorResult && evaluatorResult.usage ? 'yes' : 'no')

  # If LLM call failed, create a dummy validation result
  * eval
    """
    if (!llmCallSucceeded) {
      karate.log('=== SMARTRESPONSE LLM CALL FAILED - Creating fallback result ===');
      evaluatorResult = evaluatorResult || {};
      evaluatorResult.parsedContent = {
        pass: false,
        scores: { relevance: 0, faithfulness: 0, instructionCompliance: 0, semanticCloseness: 0 },
        responseType: 'error',
        productsReferenced: 'Unknown',
        issues: ['LLM API call failed: ' + (evaluatorResult.error ? evaluatorResult.error.message : 'Unknown error')],
        summary: 'SmartResponse evaluation could not be performed due to API error'
      };
    } else {
  ...
  # Record usage - directly write to CSV using Java (for smartResponse AI cost tracking)
  * print '=== BEFORE SMARTRESPONSE USAGE RECORDING - evaluatorResult type:', typeof evaluatorResult
  ...
  * eval
    """
    karate.log('=== INSIDE SMARTRESPONSE USAGE RECORDING EVAL ===');
    ...
    if (evaluatorResult && evaluatorResult.usage) {
      try {
        ...
        karate.log('=== RECORDING SMARTRESPONSE USAGE ===');
        ...
        karate.log('=== SMARTRESPONSE USAGE RECORDED SUCCESSFULLY ===');
      } catch (e) {
        karate.log('ERROR recording smartResponse usage:', e);
        karate.log('Error message:', e.message);
        karate.log('Error stack:', e.stack);
      }
    } else {
      karate.log('WARNING: No usage data found in smartResponse evaluatorResult');
      if (evaluatorResult) {
